...
with st.sidebar:
    ...
    st.markdown('<div class="sb-section">⚙️ &nbsp;Sesja</div>', unsafe_allow_html=True)
    year = st.number_input("Rok sezonu", min_value=2018, max_value=2025, value=2024, step=1)
...
if run_btn:
    ...
    st.session_state["results"] = None  # Wyczyść poprzednie

    # Wyczyść cache wykresów z poprzedniej sesji
    ...
    try:
        ...
        figs: dict[str, object] = {}  # {key: go.Figure | None}

        if mods_cfg["telemetry"]:
            status.info("📈 Generowanie wykresu telemetrii...")
            figs["telemetry"] = mods["plot_telemetry_interactive"](
        ...
        if mods_cfg["track"]:
            ...
            progress.progress(94, text="Mapy toru gotowe...")
        ...
        status.success(
            f"✅ Analiza zakończona! Wygenerowano {len([v for v in figs.values() if v is not None])} wykresów."
        )
...
if results is None:
    ...
    features = [
        ...
        ("🗺️", "Mapa toru",    "Dominacja · Gradient prędkości · Biegi na torze",    "#3bd971"),
        ("📊", "Podsumowanie",  "Tabela wyników · Profile kierowców · Czasy sektorów","#f9c74f"),
    ]
    cols = st.columns(3)
    for i, (icon, title, desc, accent) in enumerate(features):
        with cols[i % 3]:
            st.markdown(f"""
            <div class="feature-card delay-{(i % 6) + 1}" style="--accent:{accent}">
    ...
    <div class="note-card fade-in delay-6">
        ...
        Dane pobierane przez FastF1 i cache'owane lokalnie.
    </div>
    """, unsafe_allow_html=True)
    st.stop()
...
# ── TABS ───────────────────────────────────────────────────────────────────────
tab_names = ["📊 Podsumowanie", "📈 Telemetria", "🔄 Zakręty",
             "🏁 Sektory", "🎯 Styl jazdy", "🗺️ Mapa toru", "📋 Info"]
...
with tabs[2]:
    ...
    if fig is not None:
        ...
        if corner_analysis and corner_analysis.corners:
            st.markdown(f'<div class="f1-subheader">Dane zakrętów ({len(corner_analysis.corners)} wykryto)</div>', unsafe_allow_html=True)
            rows = []
            for corner in corner_analysis.corners:
                row = {"Zakręt": f"T{corner['id']}", "Dystans [m]": f"{corner['distance']:.0f}"}
                for drv in drivers_data:
                    events_map = {e.corner_id: e for e in corner_analysis.driver_corners.get(drv, [])}
                    ev = events_map.get(corner["id"])
    ...
    if fig_radar is not None or fig_bars is not None:
        ...
        with col_r:
            if fig_radar is not None:
                st.markdown('<div class="f1-subheader">Radar stylu jazdy</div>', unsafe_allow_html=True)
                st.plotly_chart(fig_radar, use_container_width=True)
        with col_b:
            if fig_bars is not None:
                st.markdown('<div class="f1-subheader">Porównanie metryk</div>', unsafe_allow_html=True)
                st.plotly_chart(fig_bars, use_container_width=True)

    if fingerprints:
        st.markdown('<div class="f1-subheader">Metryki stylu jazdy (0 – 100)</div>', unsafe_allow_html=True)
        metric_rows = []
...
with tabs[5]:
    ...
    elif st.session_state["params"]["mods"]["track"]:
        st.warning("⚠️ Brak danych GPS — mapy toru niedostępne dla tej sesji.")
